...
<title>MemberController서블릿에서 request에 바인딩한 ArrayList를 얻어 현재 화면에 출력</title>
	...
	<c:choose>
		<%-- 회원 수정 작업후 컨트롤러에서 넘긴 msg값에 따라 작업 결과를 alert창에 출력 --%>
		...
		<%-- 회원 가입  작업후 컨트롤러에서 넘긴 msg값에 따라 작업 결과를 alert창에 출력 --%>
	...
	<c:when test="${requestScope.membersList != null}">
		<c:forEach var="membervo" items="${requestScope.membersList}">
			<tr align="center">
				...
				<td><a href="${contextPath}/member/modeMemberForm.do?id=${membervo.id}">수정</a></td>
